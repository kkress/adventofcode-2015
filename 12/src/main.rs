...
fn sum_values(data: &Json) -> i64 {
    return match data {
        ...
        &Json::String(_) => 0,
        &Json::Boolean(_) => 0,
        &Json::Array(ref a) => a.iter().fold(0, |acc, ref x| acc + sum_values(&x)),
        &Json::Object(ref o) => o.values().fold(0, |acc, ref x| acc + sum_values(&x)),
        &Json::Null => 0,
    };
}
...
fn sum_values_unred(data: &Json) -> i64 {
    return match data {
        ...
        &Json::String(_) => 0,
        &Json::Boolean(_) => 0,
        &Json::Array(ref a) => a.iter().fold(0, |acc, ref x| acc + sum_values_unred(&x)),
        &Json::Object(ref o) => {
...
#[test]
fn nested_unred() {
    let data = Json::from_str(r#"{"a": 1, "b": {"a":[1,2,3,4,5,6], "a": "red", "b": 1}, "c":30}"#)
        .unwrap();
