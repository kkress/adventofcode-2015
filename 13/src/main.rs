...
fn score(names: &Vec<String>, happy: &HashMap<(String, String), i32>) -> i32 {
   ...
   for i in 0..names.len() {
      ...
      let left = if i == 0 { &names[names.len()-1] } else { &names[i-1] };
      let right = if i == &names.len()-1 { &names[0] } else {&names[i+1] };
      let score = happy.get(&(curr.clone(), left.clone())).unwrap() + happy.get(&(curr.clone(), right.clone())).unwrap();
      total += score;
   }
   total
}
